...
public class U_01 {
	...
	String addUserPassword = "Password#123";
	...
	public void test() throws Exception {
		...
		assertEquals("Adding user was successful", addUserMessage.getText());
		
		waitForPageLoad(driver, driver.findElement(By.id("addUserConfirm")));
		driver.findElement(By.id("addUserConfirm")).click();
		
		/*Thread.sleep(1500);
		// Check if NewUser is updated in UserTable
		WebElement table = driver.findElement(By.id("adminTableU"));
		int userTableRowNum = table.findElements(By.tagName("tr")).size();
		
		String userXPath = "//*[@id='adminTableU']/tbody/tr[" + userTableRowNum + "]/td[";
		
		WebElement firstName = driver.findElement(By.xpath(userXPath + "4]"));
		assertEquals(firstName.getText(), addUserFirstName);
		
		WebElement lastName = driver.findElement(By.xpath(userXPath + "5]"));
		assertEquals(lastName.getText(), addUserLastName);
		
		WebElement email = driver.findElement(By.xpath(userXPath + "6]"));
		assertEquals(email.getText(), addUserEmail); */
	}
